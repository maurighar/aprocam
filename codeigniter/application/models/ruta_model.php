...
class Ruta_model extends CI_Model {
	...
	public function cliente($cuit_buscar,$nombre){
		$this->db->where('cuit',$cuit_buscar);
		$consulta = $this->db->get('clientes');

		if ($consulta->num_rows() === 0) {
			$this->db->set('cuit', $cuit_buscar);
	...
	public function cliente_id($id_buscar){
	...
	public function actualizar_obs_cuit($id,$obs){
		$this->db->set('obs', $obs);
		$this->db->where('id', $id);
